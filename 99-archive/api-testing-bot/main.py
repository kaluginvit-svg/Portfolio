...
from app.keyboards import get_ai_selection_keyboard, get_remove_keyboard
from app.state import AI_AGENT_CHOICE
...
async def cmd_start(message: types.Message) -> None:
	...
	# Установить выбор по умолчанию на OpenRouter
	AI_AGENT_CHOICE[user_id] = "openrouter"
	
	welcome_text = (
		"Привет! 👋\n\n"
		"Я AI-ассистент. Задавай мне вопросы, и я постараюсь помочь!\n\n"
		"Выбери AI-агента для работы:"
	)
...
async def select_openrouter(message: types.Message) -> None:
	user_id = message.from_user.id
	AI_AGENT_CHOICE[user_id] = "openrouter"
	logger.info(f"User {user_id} selected OpenRouter")
	
	await message.answer(
		"✅ Выбран OpenRouter\n\n"
		"Теперь я буду отвечать используя OpenRouter. Пиши свой вопрос! 💬",
		reply_markup=get_remove_keyboard()
	)


@dp.message(F.text == "🟡 Chutes")
async def select_chutes(message: types.Message) -> None:
	user_id = message.from_user.id
	AI_AGENT_CHOICE[user_id] = "chutes"
	logger.info(f"User {user_id} selected Chutes")
	
	await message.answer(
		"✅ Выбран Chutes\n\n"
		"Теперь я буду отвечать используя Chutes. Пиши свой вопрос! 💬",
		reply_markup=get_remove_keyboard()
	)


@dp.message(F.text == "🟢 GigaChat")
async def select_gigachat(message: types.Message) -> None:
	user_id = message.from_user.id
	AI_AGENT_CHOICE[user_id] = "gigachat"
	logger.info(f"User {user_id} selected GigaChat")
	
	await message.answer(
		"✅ Выбран GigaChat\n\n"
		"Теперь я буду отвечать используя GigaChat. Пиши свой вопрос! 💬",
		reply_markup=get_remove_keyboard()
	)
...
async def cmd_help(message: types.Message) -> None:
	...
	help_text = """
🤖 Доступные команды:
/start - Начать работу (выбрать AI агента)
/ping - Проверка соединения
/switch - Переключиться на другого AI агента
/help - Справка
...
async def cmd_switch(message: types.Message) -> None:
	logger.info("Command /switch from user=%s", message.from_user.id)
	
	switch_text = "Выбери AI-агента:"
	await message.answer(switch_text, reply_markup=get_ai_selection_keyboard())
...
async def handle_user_message(message: types.Message) -> None:
	...
	# Получить выбранного AI агента, по умолчанию OpenRouter
	selected_agent = AI_AGENT_CHOICE.get(user_id, "openrouter")
	agent_name = {
		"openrouter": "OpenRouter",
		"chutes": "Chutes",
		"gigachat": "GigaChat"
	}.get(selected_agent, "OpenRouter")
	
	# Отправить сообщение об обработке
	...
	try:
		...
		response = await call_ai_api(user_text, selected_agent)
